(custom-set-variables
 ...
 '(ftgp-remotes
   (quote
    ("192.168.9.113"
     ("vbox Win10" "http://192.168.122.95")
     ("dev-nicolas" "https://dev-nicolas.foretagsplatsen.se")
     ("dev-camille" "https://dev-camille.foretagsplatsen.se")
     ("dev-ben" "https://dev-ben.foretagsplatsen.se")
     ("dev-damien" "https://dev-damien.foretagsplatsen.se")
     ("dev-henrik" "https://dev-henrik.foretagsplatsen.se")
     ("dev-seckin" "https://dev-seckin.foretagsplatsen.se")
     ("dev-mikael" "https://dev-mikael.foretagsplatsen.se")
     ("dev-master" "https://dev-master.foretagsplatsen.se")
     ("staging-ftgp-test" "https://staging-ftgp-test.foretagsplatsen.se")
     ("FTGP Prod" "https://web.foretagsplatsen.se")
     ("Ben Brest" "http://192.168.0.10:8888")
     ("Mikael Stockholm" "http://192.168.9.123"))))
 '(gdb-enable-debug t)
 ...
 '(ledger-reports
   (quote
    (("Budget" "%(binary) bal -f %(ledger-file) --explicit --pedantic --empty -S -T ^assets:budget --balance-format='%(ansify_if(justify(scrub(display_total), 20, 20 + int(prepend_width), true, color), bold if should_bold)) %-(ansify_if(ansify_if(justify(depth_spacer + partial_account(), 30, 30, false, color), blue if color), bold if should_bold)) %(ansify_if(note, yellow if color)) \\n%/ %$1\\n%/ %(prepend_width ? \" \" * int(prepend_width) : \"\") --------------------\\n'")
     ...
     ("Monthly cash flow" "%(binary) reg -f %(ledger-file) --explicit --pedantic --real ^assets:current --monthly --collapse")
     ("Monthly balance" "%(binary) reg -f %(ledger-file) --explicit --pedantic --real  ^assets ^liabilities ^equity --monthly --collapse")
     ("Cash Flow" "%(binary) reg -f %(ledger-file) --explicit --pedantic --real --related --invert ^assets:currentq -p %(month)")
 ...
 '(notmuch-saved-searches
   (quote
    ((:name "To me" :query "tag:unread and (to:petton or cc:petton or to:nicolas or cc:nicolas)" :key "m")
 ...
 '(package-selected-packages
   (quote
    (solarized-theme lsp-ui company-lsp dap-mode bui tree-mode lsp-mode ht csv transient prettier-js company-go rainbow-mode go-mode request yasnippets sudo-edit htmlize ox-jira closql emacsql-sqlite emacsql devdocs stream test-simple loc-changes load-relative zoom-frm frame-cmds frame-fns smex json-snatcher json-reformat org-caldav klassified hierarchy gulp-task-runner eslintd-fix debbugs alert log4e gntp buffer-move prodigy flycheck-ledger yaml-mode minions all-the-icons memoize ws-butler macrostep shell-switcher password-store-otp password-store tablist ox-twbs csharp-mode auto-complete popup esxml less-css-mode xref-js2 company-tern widgetjs tern amd-mode makey projectile js2-refactor shut-up elisp-refs loop dash-functional flyspell-correct embrace duplicate-thing drag-stuff queue paredit sesman pkg-info epl clojure-mode magit-popup git-commit with-editor treepy graphql async buffer-watcher f s beginend packed bind-key)))
